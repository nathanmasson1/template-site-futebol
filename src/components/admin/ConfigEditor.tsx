...
import { Save, AlertCircle, Loader2, Image as ImageIcon, Sparkles, Eye, EyeOff } from 'lucide-react';
...
export default function ConfigEditor({ initialLogo = '' }: ConfigEditorProps) {
    ...
    const [pendingFooterLogo, setPendingFooterLogo] = useState<File | null>(null);
    ...
    useEffect(() => {
        githubApi('read', 'src/data/siteConfig.json')
            .then(data => {
                const remoteConfig = JSON.parse(data?.content || "{}");
                setConfig({ ...remoteConfig, logo: remoteConfig.logo || initialLogo });
                setFileSha(data.sha);
            })
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (pendingFooterLogo) {
                triggerToast('Enviando logo do footer...', 'progress', 40);
                const base64Content = await fileToBase64(pendingFooterLogo);
                const fileExt = pendingFooterLogo.name.split('.').pop() || 'png';
                const ghPath = `public/uploads/${Date.now()}-footer-logo.${fileExt}`;
                await githubApi('write', ghPath, { content: base64Content, isBase64: true, message: 'CMS: Upload Footer Logo' });
                configCopy.footerLogo = ghPath.replace('public', '');
            }
            if (pendingFavicon) {
            ...
            setFileSha(res.sha); setConfig(configCopy); setPendingLogo(null); setPendingFooterLogo(null); setPendingFavicon(null);
    ...
    return (
        <form onSubmit={handleSave} className="space-y-8 pb-32 max-w-3xl">
            ...
            <div className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-8 items-start">
                        ...
                        <div className="w-full">
                            <label className={labelClass}>Logo do Footer</label>
                            <label className="group relative border-2 border-dashed border-slate-300 hover:border-violet-500 bg-slate-900 hover:bg-slate-800 rounded-2xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all text-center h-48">
                                <input type="file" accept="image/*" className="hidden" onChange={(e) => {
                                    const file = e.target.files?.[0];
                                    if (file) { setPendingFooterLogo(file); setConfig({ ...config, footerLogo: URL.createObjectURL(file) }); }
                                }} />
                                {config?.footerLogo ? (
                                    <img src={config.footerLogo} alt="Logo do footer" className="max-h-24 w-auto object-contain mb-4 group-hover:scale-105 transition-transform" />
                                ) : (
                                    <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center text-slate-400 shadow-sm mb-3 group-hover:text-violet-200 transition-colors">
                                        <ImageIcon className="w-8 h-8" />
                                    </div>
                                )}
                                <span className="text-sm font-semibold text-white group-hover:text-violet-100 transition-colors">
                                    {config?.footerLogo ? 'Trocar Logo do Footer' : 'Enviar Logo do Footer'}
                                </span>
                                <span className="text-[10px] text-slate-400 mt-1">Ideal para fundo escuro</span>
                            </label>
                        </div>
                        <div className="w-full">
            ...
            {/* IA / OpenAI */}
            <div id="ai" className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm scroll-mt-8">
                <h3 className="text-xl font-bold text-slate-900 mb-2 flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-violet-600" /> Inteligência Artificial
                </h3>
                <p className="text-sm text-slate-500 mb-6 border-b border-slate-100 pb-4">Configure o OpenAI para gerar reviews automaticamente no editor.</p>
                <div className="space-y-4">
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800">
                        <strong>⚠️ Atenção:</strong> A chave fica salva em <code className="bg-amber-100 px-1 rounded">siteConfig.json</code> (commitada no GitHub). Para mais segurança, defina <code className="bg-amber-100 px-1 rounded">OPENAI_API_KEY</code> como variável de ambiente no Vercel — ela tem precedência sobre este campo.
                    </div>
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                autoComplete="off"
                            />
                            <button type="button" onClick={() => setShowApiKey(!showApiKey)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-700">
                                {showApiKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                            </button>
                        </div>
                        <p className="text-[10px] text-slate-400 mt-1">Obtenha em <a href="https://platform.openai.com/api-keys" target="_blank" rel="noopener" className="text-violet-600 underline">platform.openai.com/api-keys</a></p>
                    </div>
                    <div>
            ...
            <div className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm">
                ...
                <div className="space-y-4">
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                        ...
                        <p className="text-xs text-emerald-600 mb-3">O sitemap é atualizado a cada build/deploy com as páginas públicas e posts do site. Rotas internas como /admin e /api ficam fora.</p>
                        {config?.url ? (
