...
import { isPagePath, withInternalTrailingSlash } from './lib/urls';
...
export const onRequest = defineMiddleware(async (context, next) => {
    ...
    if (!pathname.startsWith('/admin') && !pathname.startsWith('/api/')) {
        const legacyCategory = pathname.replace(/\/+$/, '') === '/blog'
            ? context.url.searchParams.get('cat')?.trim().toLowerCase()
            : '';
        const categoryRoutes = new Set(['brasileirao', 'analises', 'futebol-feminino', 'categorias-de-base']);

        if (legacyCategory && categoryRoutes.has(legacyCategory)) {
            return context.redirect(`/${legacyCategory}/`, 301);
        }
        ...
        for (const r of redirects) {
            ...
            if (r.enabled && normFrom && r.to && (normFrom === normPath || r.from === pathname)) {
                return context.redirect(withInternalTrailingSlash(r.to), r.type || 301);
            }
        }
