...
interface SettingsGSCProps {
    initialVerificationTag?: string;
    initialSiteUrl?: string;
}

export default function SettingsGSC({ initialVerificationTag = '', initialSiteUrl = '' }: SettingsGSCProps) {
    const [verificationTag, setVerificationTag] = useState(initialVerificationTag);
    ...
    const handleSave = async () => {
        if (!fullConfig) {
            setError('Aguarde a sincronização das configurações antes de salvar.');
            return;
        }

        if (verificationTag.trim() && !extractGoogleSiteVerification(verificationTag)) {
            setError('A meta tag de verificação é inválida. Cole a tag do Google ou somente o valor de content.');
            return;
        }
    ...
    return (
        <div className="max-w-2xl space-y-6">
            ...
            {loading && (
                <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4 flex items-center gap-3 text-violet-700">
                    <Loader2 className="w-4 h-4 animate-spin shrink-0" />
                    <p className="text-sm font-medium">Sincronizando as configurações protegidas...</p>
                </div>
            )}
            ...
            <div className="flex gap-3">
                ...
                <button
                    type="button"
                    onClick={handleSave}
                    disabled={saving || loading || !fullConfig}
